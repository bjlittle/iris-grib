...
setup_args = dict(
    ...
    # XXX: This is a royal dependency mess at the moment. The PyPI eccodes package 2.10.0 is
    # a wheel for win_amd64 only, so we'll only add that as a dependence for the relevant
    # platform, otherwise unfortunately it's up to the user to provision themselves until
    # ECMWF and PyPI provide the suitable package platform coverage.
    install_requires = ['scitools-iris>=2.0.*'] + ['eccodes'] if 'win' in sys.platform else [],
    extras_require = {
